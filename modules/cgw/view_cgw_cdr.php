...
if($edit_tbl == "cgw_cdr"){
	...
	$content_arr = array("UniqueID","ServiceID","ChargingType", "CGWID", "Ano", "Bno", "Direction", "RateID", "ResultCode", "Amount", "Purpose", "PackageID","BillingNode");
	if($short_code != null AND $short_code != ' ' AND $service_id != null AND  $service_id != ' ')
	{
		...
		$load_qry = "SELECT `UniqueID`,`ServiceID`, `ChargingType`, `CGWID`, `Ano`, `Bno`, `Direction`, `RateID`,  `ResultCode`,  `Amount`, `Purpose`, " .
...
<script>
	$(document).ready(function() {
		...
		$("#view_<?php echo $edit_tbl; ?> .action_post").click(function() {
			...
			var trRow = $("#"+id).parent("td").parent("tr");
			/*
			if(action == "edit"){
				$("#action_id").val(value);
