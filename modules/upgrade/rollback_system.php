...
function db_import($cn){
	...
	$mysqlPassword = $MYPASSWORD;
	$mysqlDatabaseName = $MYDB;  
	$mysqlImportFilename = $LATEST_FILES_BACKUP_PATH."/".$available_version."/DB/".$MYDB.".sql";
...
function files_import($cn){
	...
	$backup_command = "sudo cp -r ".$LATEST_FILES_BACKUP_PATH.$available_version."/files/* ".$CURRENT_FILE_HOSTING_PATH;
	try {
		$is_error = 0;
		shell_exec($backup_command);
