...
if (!empty($_POST) && $_POST['operator_db'])
{
	$cn = connectDB();
	...
	if($action == "insert" ){
		...
		$qry = "update `service` set ";		
	} elseif($action == "delete"){
		//$qry = "update `timeslot` set is_active='inactive' where is_active='active' and `id`='".$action_id."'  and `id`='".$action_id."'";
	...
	foreach($_POST as $pname => $pvalue){
		...
		if(!($pname == "action" || $pname == "action_id" || $pname == "operator_db")){
			if($count>0){
				$seperator = ",";
			}
