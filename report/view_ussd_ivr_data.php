...
if($edit_tbl == "ivr_data"){
	$count_qry = "SELECT count(PageName) FROM ivrmenu_copy where PageName='$page_id' and Service='$service_id'";
	...
	$load_qry = "SELECT * FROM ivrmenu_copy WHERE PageName='$page_id' and Service='$service_id'";
	...
	$(document).ready(function() {
		...
		function page_load(page,per_page){
			...
			var data = "page="+page+"&per_page="+per_page+"&tbl="+tbl+"&service_id="+get_value("#service_id")+"&page_id="+get_value("#page_id");
				...
				$.ajax({
					type: "POST",
					url: "report/view_ussd_ivr_data.php",
					data: data,
					cache: false,
