...
<h1>CMS : USER PANEL</h1>
	<div class="breadcrumb">
		<ul>
            ...
            <li><a href="#">Revenue Reporting</a></li>
		</ul>
	</div>
	<div class="content">
      <div class="halfpan fl">
         <h2>Revenue Reporting</h2>
          ...
          <input type="hidden" value="revenue_report" id="page_type" />
     ...
     <div class="tblcss" id="view_revenue_report" style="height:1024px;overflow:auto;"></div>
<script>
$( document ).ready(function() {
		...
		$("#submit").click(function(){
			...
			{
				var list=restore_fields("modules/cms/show_revenue_report",['operator_id','service_id','from_id','to_id','trigger']);
				// console.log(list);	
				if(list && list.indexOf("tr")>-1 ){ 
				   $("#view_revenue_report").empty().html(list);
				   $("#csv").show();
				}else if(list==1){  // list==1
					$("#view_revenue_report").empty().html("<p>No Data Available</p>");
					$("#csv").hide();
				}else{ //databse connection error
					$("#view_revenue_report").empty().html(list);
					$("#csv").hide();
				}
			}else{
				$("#view_revenue_report").html("");
				$("#csv").hide();
			}
		});	
...
$("#csv").click(function(){
	...
	window.open("modules/cms/show_revenue_report.php?"+param);
});
